...
public class UISettingsAssets : MonoBehaviour
{
    ...
    public void CopyAllLoadedAssets()
    {
        ...
        if (path != null && path.Length > 0 && !string.IsNullOrEmpty(path[0]))
        {
            ...
            UmaViewerUI.Instance.ShowMessage(LocalizationManager.Get("msg.files_copied", LoadedAssetEntries.Count), UIMessageType.Success);
        }
#else
    ...
    public void CopyCurrentUmaAssets()
    {
        ...
        {
            UmaViewerUI.Instance.ShowMessage(LocalizationManager.Get("msg.no_umamusume_loaded"), UIMessageType.Warning);
            return;
        }
        ...
        if (path != null && path.Length > 0 && !string.IsNullOrEmpty(path[0]))
        {
            ...
            UmaViewerUI.Instance.ShowMessage(LocalizationManager.Get("msg.files_copied", assets.Count), UIMessageType.Success);
        }
#else
        UmaViewerUI.Instance.ShowMessage(LocalizationManager.Get("msg.not_supported"), UIMessageType.Warning);
#endif
