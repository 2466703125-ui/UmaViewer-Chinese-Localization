...
public class Config
{
    ...
    // 所有 getter 属性加空值保护，防止 Config 构造时 LocalizationManager 还未初始化
    
    [Newtonsoft.Json.JsonIgnore]
    public string DBBaseKeyTip => LocalizationManager.Instance != null 
        ? LocalizationManager.Get("config.db_base_key_tip") 
        : "Base key to read game database files";
    
    [Newtonsoft.Json.JsonIgnore]
    public string DBKeyTip => LocalizationManager.Instance != null 
        ? LocalizationManager.Get("config.db_key_tip") 
        : "Key to read game database files";
    
    [Newtonsoft.Json.JsonIgnore]
    public string GlobalDBKeyTip => LocalizationManager.Instance != null 
        ? LocalizationManager.Get("config.global_db_key_tip") 
        : "Key to read game database files (for Global)";
    
    [Newtonsoft.Json.JsonIgnore]
    public string ABKeyTip => LocalizationManager.Instance != null 
        ? LocalizationManager.Get("config.ab_key_tip") 
        : "Key to read asset bundles";
    
    [Newtonsoft.Json.JsonIgnore]
    public string LanguageTip => LocalizationManager.Instance != null 
        ? LocalizationManager.Get("config.language_tip") 
        : "UI Language. Options: 0 - English, 1 - 中文";
    
    public Language Language = Language.En;

    [Newtonsoft.Json.JsonIgnore]
    public string CharacterNameLanguageTip => LocalizationManager.Instance != null 
        ? LocalizationManager.Get("config.character_name_language_tip") 
        : "Character name display language. Options: 0 - 日本語, 1 - English, 2 - 中文";
    
    public CharacterNameLanguage CharacterNameLanguage = CharacterNameLanguage.Japanese;

    [Newtonsoft.Json.JsonIgnore]
    public string RegionTip => LocalizationManager.Instance != null 
        ? LocalizationManager.Get("config.region_tip") 
        : "Game region. Region options: 0 - Global, 1 - Japan";
    
    public Region Region = Region.Jp;

    [Newtonsoft.Json.JsonIgnore]
    public string DownloadMissingResourcesTip => LocalizationManager.Instance != null 
        ? LocalizationManager.Get("config.download_tip") 
        : "true/false. automatically download missing files, which may require a VPN.";
    
    public bool DownloadMissingResources = true;

    [Newtonsoft.Json.JsonIgnore]
    public string MainPathTip => LocalizationManager.Instance != null 
        ? LocalizationManager.Get("config.main_path_tip") 
        : @"Path to game folder, eg. D:/Backup/Cygames/umamusume";
    
    public string MainPath = "";

    [Newtonsoft.Json.JsonIgnore]
    public string WorkModeTip => LocalizationManager.Instance != null 
        ? LocalizationManager.Get("config.work_mode_tip") 
        : "Affects how application work, options: 0 - work with game client, 1 - work without game client(slow)";
    
    public WorkMode WorkMode = WorkMode.Default;

    [Newtonsoft.Json.JsonIgnore]
    public string VmdKeyReductionLevelTip => LocalizationManager.Instance != null 
        ? LocalizationManager.Get("config.vmd_key_tip") 
        : "Affects the recording quality: 1 = record every frame, 2 = record every two frames, and so on.";
    
    public int VmdKeyReductionLevel = 2;

    [Newtonsoft.Json.JsonIgnore]
    public string AntiAliasingTip => LocalizationManager.Instance != null 
        ? LocalizationManager.Get("config.anti_aliasing_tip") 
        : "Display, screenshot antialiasing level. 0 - no AA, 1 - 2x MSAA, 2 - 4x MSAA, 3 - 8x MSAA";
